	...
	"full": {
		expr: model.NewsFindExpr{
			...
			OrderBy: map[string]bool{
				...
				model.TableNewsColumnContent: false,
			},
		},
		query: "SELECT " + join(model.TableNewsColumns, 0) + " FROM example.news AS t0 WHERE t0.content=$1 AND t0.continue=$2 AND t0.created_at=$3 AND t0.lead=$4 AND t0.meta_data=$5 AND t0.score=$6 AND t0.title=$7 AND t0.updated_at=$8 AND t0.views_distribution=$9 ORDER BY title, content DESC  OFFSET $10  LIMIT $11 ",
	},
}
